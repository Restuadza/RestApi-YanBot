...
function otakudesu(judul){
	...
	axios.get('https://otakudesu.lol/?s=' + judul + '&post_type=anime')
	.then(({ data }) => {
...
function ongoing(){
	return new Promise((reject,resolve) => {
		axios.get('https://otakudesu.lol/ongoing-anime/').then(({ data}) => {
			const $ = cheerio.load(data)
			const result = [];
			const img = [];
			const epz = [];
...
function komiku(judul) {
	return new Promise(async(resolve,reject) => {
	axios.get('https://komiku.id/cari/?post_type=manga&s=' + encodeURIComponent(judul))
	.then(({ data }) => {
	const $ = cheerio.load(data)
	const img = []; 
	const or = [];
...
function igstory(username){
	return new Promise(async(resolve, reject) => {
		axios.request({
			url: 'https://saveinsta.app/id/instagram-story-download.php',
			method: 'GET',
			headers:{
				"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
				"cookie": "PHPSESSID=ugpgvu6fgc4592jh7ht9d18v49; _ga=GA1.2.1126798330.1625045680; _gid=GA1.2.1475525047.1625045680; __gads=ID=92b58ed9ed58d147-221917af11ca0021:T=1625045679:RT=1625045679:S=ALNI_MYnQToDW3kOUClBGEzULNjeyAqOtg"
